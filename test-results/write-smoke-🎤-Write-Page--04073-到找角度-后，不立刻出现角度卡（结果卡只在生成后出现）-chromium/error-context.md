...
- Name: write-smoke.spec.ts >> 🎤 Write Page - 精细化冒烟测试（已修复 bug 验证） >> 点击"发送到找角度"后，不立刻出现角度卡（结果卡只在生成后出现）
- Location: write-smoke.spec.ts:80:7
...
Call log:
  - waiting for locator('text=⭐ 推荐前提') to be visible
      ...
      - generic [ref=e35]:
        - generic [ref=e36]:
          - generic [ref=e37]:
            ...
            - textbox "输入一段素材： 一件事、一句抱怨、一个观察、一段情绪…… 📝 示例： • 我发现同事离职后，工位像被系统自动回收一样 • 我妈总觉得我写脱口秀不算正经工作 • 成年人最擅长的事，就是把委屈说成体面" [disabled] [ref=e44]:
              - /placeholder: "输入一段素材：\n一件事、一句抱怨、一个观察、一段情绪……\n\n📝 示例：\n• 我发现同事离职后，工位像被系统自动回收一样\n• 我妈总觉得我写脱口秀不算正经工作\n• 成年人最擅长的事，就是把委屈说成体面"
              - text: 我妈总觉得我写脱口秀不算正经工作
            - generic [ref=e45]:
              - generic [ref=e46]:
                - generic [ref=e47]: 已输入 16 字
                - generic [ref=e48]: 至少 20 字可开始，建议 50–200 字
              - button "分析中..." [disabled] [ref=e49]
            - paragraph [ref=e50]: 生成后自动保存到右侧创作会话
          - generic [ref=e51]:
            - generic [ref=e54]: AI 分析中
            - generic [ref=e56]: 正在分析素材，马上给你前提候选…
        - generic [ref=e58]:
          - generic [ref=e59]:
            - paragraph [ref=e60]: 这个工具做什么？
            - button "✕" [ref=e61] [cursor=pointer]
          - paragraph [ref=e62]:
            - text: 把一段素材、情绪、观察，转化成
            - strong [ref=e63]: 可以上台说
            - text: 的喜剧前提。
          - generic [ref=e64]:
            - generic [ref=e65]:
              - generic [ref=e66]: ✓
              - paragraph [ref=e67]: 识别主题和态度
            - generic [ref=e68]:
              - generic [ref=e69]: ✓
              - paragraph [ref=e70]: 提炼核心矛盾
            - generic [ref=e71]:
              - generic [ref=e72]: ✓
              - paragraph [ref=e73]: 生成 5 个前提候选
            - generic [ref=e74]:
              - generic [ref=e75]: ✓
              - paragraph [ref=e76]: 推荐最优前提
            - generic [ref=e77]:
              - generic [ref=e78]: ✓
              - paragraph [ref=e79]: 给出场景建议
          - paragraph [ref=e81]: 输入素材越具体，提炼的前提越精准
  - alert [ref=e82]
```
...
> 87  |     await page.waitForSelector('text=⭐ 推荐前提', { timeout: 15000 });
  ...
  153 |     if (hasResult) {
  154 |       // 点击"发送到找角度"
  155 |       const moreBtn = page.locator('text=更多 ▾').first();
  156 |       if (await moreBtn.isVisible({ timeout: 3000 })) {
  157 |         await moreBtn.click();
  158 |         await page.waitForTimeout(300);
  159 |         const sendBtn = page.getByText('发送到找角度').first();
  160 |         if (await sendBtn.isVisible()) {
  161 |           await sendBtn.click();
  162 |           await page.waitForTimeout(800);
  163 |           // Tab 切换后，Session Panel 中的下一张卡应该有来源链
  164 |           // 由于需要生成角度结果，这里只验证 toast 和 Tab 切换
  165 |           await expect(page.getByText('已带入')).toBeVisible({ timeout: 2000 });
  166 |           console.log('✅ handoff 来源链 toast 正常');
  167 |         }
  168 |       }
  169 |     } else {
  170 |       console.log('⚠️ 前提生成失败，跳过来源链验证');
  171 |     }
  172 |   });
  173 | 
  174 |   // ── Feature: 无 console error ───────────────────────────────────
  175 |   test('页面加载和基本操作零 console error', async ({ page }) => {
  176 |     const errors: string[] = [];
  177 |     page.on('console', msg => {
  178 |       if (msg.type() === 'error') errors.push(msg.text());
  179 |     });
  180 | 
  ...
  184 | 
  185 |     // 基本填入操作
  186 |     await page.locator('textarea').first().fill('成年人最擅长的事，就是把委屈说成体面');
